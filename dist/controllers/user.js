...
import { handleFailedLogin, handleSuccessfulLogin, } from "../middleware/validator.js";
...
export const passwordChangeEmailVerify = async (req, res) => {
    try {
...
export const passwordChangeOtpVerify = async (req, res) => {
    try {
...
export const logoutActivity = async (req, res) => {
    try {
        const { userId } = req.body;
        const device = req.device;
        const user = await User.findById(userId);
        if (user) {
            user.activeSession = user.activeSession.filter((session) => session.deviceId !== device);
            const timestamp = Date.now();
            const loginActivities = await UserActivity.find({
                user: user._id,
                device: device,
            });
            const index = loginActivities.findIndex((activity) => activity.type === "login");
            if (index !== -1) {
